...
IF NOT EXISTS (SELECT 1 FROM dbo.Vehicles WHERE PlateNo = '29X1-123.45')
    ...
    SELECT AccountID, '29X1-123.45', 'MOTORCYCLE', N'Honda', N'Đen' FROM dbo.Accounts WHERE Email = 'khoiotaku1907@gmail.com';

IF NOT EXISTS (SELECT 1 FROM dbo.Vehicles WHERE PlateNo = '51A-123.45')
    INSERT dbo.Vehicles(AccountID, PlateNo, VehicleType, Brand, Color)
    SELECT AccountID, '51A-123.45', 'CAR', N'Toyota', N'Trắng' FROM dbo.Accounts WHERE Email = 'khoiotaku1907@gmail.com';

IF NOT EXISTS (SELECT 1 FROM dbo.Vehicles WHERE PlateNo = '43A-999.11')
    INSERT dbo.Vehicles(AccountID, PlateNo, VehicleType, Brand, Color)
    SELECT AccountID, '43A-999.11', 'MOTORCYCLE', N'Yamaha', N'Xanh' FROM dbo.Accounts WHERE Email = 'user02@gmail.com';

IF NOT EXISTS (SELECT 1 FROM dbo.Vehicles WHERE PlateNo = '51F-888.88')
    INSERT dbo.Vehicles(AccountID, PlateNo, VehicleType, Brand, Color)
    SELECT AccountID, '51F-888.88', 'CAR', N'Kia', N'Đen' FROM dbo.Accounts WHERE Email = 'user02@gmail.com';

IF NOT EXISTS (SELECT 1 FROM dbo.Vehicles WHERE PlateNo = '59A-123.45')
    INSERT dbo.Vehicles(AccountID, PlateNo, VehicleType, Brand, Color)
    SELECT AccountID, '59A-123.45', 'MOTORCYCLE', N'Honda', N'Đỏ' FROM dbo.Accounts WHERE Email = 'levancuong@gmail.com';
GO
